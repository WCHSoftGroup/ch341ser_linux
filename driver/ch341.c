 ...
 * V1.7 - added supports for kernel version beyond 5.14.x
 * 		- fixed data analysis in status ep callback
 */
...
#define VERSION_DESC  "V1.7 On 2022.08.23"
...
static int get_serial_info(struct ch341 *ch341, struct serial_struct __user *info)
{
    ...
    tmp.closing_wait = ch341->port.closing_wait == ASYNC_CLOSING_WAIT_NONE ? ASYNC_CLOSING_WAIT_NONE : ch341->port.closing_wait / 10;
...
static int __init ch341_init(void)
{
    int retval;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0))
    ch341_tty_driver = tty_alloc_driver(CH341_TTY_MINORS, TTY_DRIVER_REAL_RAW |
